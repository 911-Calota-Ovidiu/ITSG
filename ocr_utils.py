...
EXPECTED_LABELS = {
    ...
    "start_date": ["data inceput", "data început", "data debut"],
    "end_date": ["data sfarsit", "data sfârșit", "data final"],
...
def extract_value_for_label(words, label_entry, max_horizontal_gap=0.7, max_vertical_gap=0.25):
    ...
    for j, w in enumerate(words):
        if j == idx:
            continue
        x, y, ww, hh = w['bbox']
        if x > lx + lw and abs(w['cy'] - lcy) < lh * 1.5 + hh * 0.5 and (x - (lx + lw)) < max_hgap:
    ...
    for j, w in enumerate(words):
        if j == idx:
            continue
        x, y, ww, hh = w['bbox']
        if y > ly + lh and abs(w['cx'] - (lx + lw / 2)) < lw * 2 and (y - (ly + lh)) < max_vgap:
            below.append((j, w))
...
def easyocr_words(image):
    #Use EasyOCR to read the full image and return list of dicts {'text','conf','bbox'}.
...
def auto_extract_structured_fields(image_path_or_array):
    # First run Tesseract for broad coverage
    words_t, _ = image_to_words(image_path_or_array)

    # Optionally run EasyOCR for handwriting / low-quality
    words_e = easyocr_words(image_path_or_array)

    # Merge results (keep all words)
    ...
    for key, entry in found_labels.items():
        val_text, _ = extract_value_for_label(words, entry)
        if (val_text is None or val_text.strip() == ''):
            if key == 'CNP':
                ...
                m = re.search(r'\b\d{2}[./-]\d{2}[./-]\d{2,4}\b', all_text)
                if m:
                    val_text = m.group(0)
